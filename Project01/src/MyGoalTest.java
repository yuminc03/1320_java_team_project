...
//		JButton goalB;//목표 메시지를 띄울 버튼
		...
		String haveMoney = Integer.toString(Book.Total);// 현재 가지고 있는 돈을 장부에서 불러와서 int타입으로 바꾼 뒤, haveMoney에 대입
		JTextField moneyT = new JTextField();// JTextField moneyT를 생성함(현재 가지고 있는 금액을 넣을 TextField).
		moneyT.setText(haveMoney);// moneyT의 텍스트를 haveMoney로 지정함
		...
		goalB.addActionListener(new ActionListener() {// goalB의 기능을 추가함
			...
			public void actionPerformed(ActionEvent arg0) {// 버튼을 클릭하면 실행함
				...
				int moneyI = Integer.parseInt(haveMoney);// haveMoney(현재 가지고 있는 돈)을 int타입으로 바꾸고 moneyI에 저장함
